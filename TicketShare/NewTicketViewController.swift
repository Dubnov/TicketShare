    ...
    @IBAction func saveTicket(_ sender: Any) {
        ...
        if self.imgImage.image != nil {
            Model.instance.saveImage(image: self.imgImage.image!, name: self.txtTitle.text!) {(url) in
                let ticket = Ticket(seller: Model.instance.getCurrentAuthUserName()!, title: self.txtTitle.text!, price: Int(self.txtPrice.text!)!, amount: Int(self.txtAmount.text!)!, eventType: 1, address: self.txtEventAddress.text!, isSold: false, description: self.txtDescription.text!, imageUrl: url)
            ...
            let ticket = Ticket(seller: Model.instance.getCurrentAuthUserName()!, title: txtTitle.text!, price: Int(txtPrice.text!)!, amount: Int(txtAmount.text!)!, eventType: 1, address: txtEventAddress.text!, isSold: false, description: txtDescription.text, imageUrl: nil)
            Model.instance.addTicket(ticket: ticket)
